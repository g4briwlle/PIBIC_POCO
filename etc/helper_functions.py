...
"""O que faz: Gera uma onda quadrada com parâmetros ajustáveis (amplitude, frequência e fase). 
Ela se baseia em uma onda senoidal para criar a onda quadrada, atribuindo valores de amplitude 
positiva ou negativa com base no valor da senoide.
Aplicação: Pode ser usada para simular sinais discretos que mudam abruptamente, talvez representando 
mudanças significativas em transações de madeira, como alterações bruscas de volume transportado entre períodos."""
...
"""O que faz: Gera uma onda senoidal, controlada por parâmetros de amplitude, frequência e fase. A onda é 
suave e periódica, representando variações regulares ao longo do tempo.
Aplicação: Pode modelar comportamentos regulares no transporte de madeira, como padrões 
sazonais ou cíclicos nas transações de uma empresa. É útil para identificar tendências periódicas 
nos dados temporais."""
    
def addnoise(signal1, noise_amp):
    return signal1 + np.random.normal(scale = noise_amp, size = len(signal1))
"""O que faz: Adiciona ruído gaussiano a um sinal dado, simulando variações aleatórias no sinal original.
Aplicação: Pode ser usada para testar a robustez de suas análises ao ruído ou simular incertezas nos dados 
das transações de madeira, permitindo verificar como pequenos desvios impactariam os resultados da análise."""
...
"""O que faz: Calcula a média de várias séries temporais usando Dynamic Time Warping (DTW). 
A função realiza várias iterações para ajustar o sinal médio com base nos caminhos de alinhamento
entre o sinal médio atual e os sinais fornecidos.
Aplicação: Essencial para análise de agrupamento ou alinhamento de séries temporais onde os sinais
podem não estar diretamente sincronizados no tempo. No seu projeto de análise de volume de madeira, 
pode ser útil para calcular médias de séries temporais de volumes de diferentes empresas, mesmo que 
os tempos de transações sejam diferentes."""
...
"""O que faz: Dado um caminho de DTW entre dois sinais, essa função gera o sinal "distorcido" 
correspondente. Ela aplica o caminho de alinhamento para ajustar os índices temporais do sinal original.
Aplicação: Útil para ver como os sinais seriam se estivessem "distorcidos" no tempo para alinhar 
com outro. No contexto do seu projeto, isso pode ajudar a alinhar séries temporais de diferentes 
empresas para comparar volumes transportados."""
...
"""Calcula a pureza de agrupamentos. A função compara os rótulos reais dos dados com os rótulos atribuídos 
por um algoritmo de clustering e determina a correspondência de cada grupo e sua pureza (a proporção 
de elementos corretamente agrupados).
Aplicação: Pode ser muito útil para avaliar a qualidade de agrupamentos de empresas/pessoas baseados em 
seus padrões de transação de madeira, ajudando a identificar comportamentos suspeitos de grupos "puros"."""
